<template>
  <div id="manageQuestionView">
    ...
    >
      ...
      >
        <IconClose v-if="popupVisible2" style="font-size: 1.2em" />
        <icon-up v-else style="font-size: 1.2em" />
      </div>
      <template #content>
        <a-menu
        ...
        >
          ...
          <a-menu-item key="1" @click="handleDelete">
            <template #icon>
              <icon-delete style="font-size: 1.2em" />
            </template>
            删除所选行
          </a-menu-item>
        </a-menu>
    ...
    <div style="box-shadow: 0 0 4px rgba(0, 0, 0, 0.2)">
      ...
      >
        ...
        <template #passRate="{ record }">
          ...
          >
            <a-statistic
              :value="(record.acceptedNum / record.submitNum) * 100"
              :precision="2"
              :value-from="0"
              :start="true"
              animation
        ...
        <!-- 标签 -->
        <template #tags="{ record }">
          <template v-if="JSON.parse(record.tags).length > 1">
            <a-trigger position="right">
              <div
                style="
                  max-width: 200px;
              ...
              >
                <a-tag
                  style="
                    ...
                    margin-right: 8px;
                  "
                  v-for="(tag, index) of JSON.parse(record.tags).slice(0, 1)"
                  :key="index"
                  :color="showTags.includes(tag) ? '#165dff' : ''"
                >
                  {{ tag }}
                </a-tag>
                <!-- 只有当标签数量大于1时才显示 icon-right -->
                <icon-right
                  v-if="JSON.parse(record.tags).length > 1"
        ...
        <template #judgeCase="{ record }">
          <a-trigger position="right">
            ...
            <template #content>
              <div class="content-basic" style="overflow: auto">
                <a-space direction="vertical">
                  <a-tag
                    style="border: 1px solid #ccc; border-radius: 10px"
...
import IconContent from "@/views/calendar/icon/icon-content.vue";
import IconExecuteTime from "@/views/calendar/icon/icon-execute-time.vue";
import IconCpu from "@/views/calendar/icon/icon-cpu.vue";
import IconStoragePlace from "@/views/calendar/icon/icon-storage-place.vue";
...
const updateUserData = async (dataList: any[]) => {
  ...
  for (let i = 0; i < dataList.length; i++) {
    ...
    if (
      ...
      item.userName = "未知用户";
      item.userAvatar =
        "https://himg.bdimg.com/sys/portraitn/item/03cde88aa5e69cabe59682e58fa3e59abce7b396df8f"; // 默认头像链接
