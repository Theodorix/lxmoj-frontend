        ...
        <a-trigger position="bl" :popup-visible="displayedSubmit_State">
          ...
          <template #content>
            <div class="demo-basic">
              ...
              <button
                class="transparent-btn"
                @click="doSubmit_State('Waiting')"
              >
                <span style="color: #1e90ff">等待审核</span>
                <img
                  v-if="Submit_State === 'Waiting'"
                  :src="require('@/assets/对.png')"
                  alt=""
                  class="small-image"
                />
              ...
              <button
                class="transparent-btn"
                @click="doSubmit_State('Wrong Answer')"
              >
              ...
              <button
                class="transparent-btn"
                @click="doSubmit_State('Memory Limit Exceeded')"
              >
              ...
              <button
                class="transparent-btn"
                @click="doSubmit_State('Time Limit Exceeded')"
              >
              ...
              <button
                class="transparent-btn"
                @click="doSubmit_State('Output Limit Exceeded')"
              >
              ...
              <button
                class="transparent-btn"
                @click="doSubmit_State('Compile Error')"
              >
              ...
              <button
                class="transparent-btn"
                @click="doSubmit_State('Execution Error')"
              >
              ...
              <button
                class="transparent-btn"
                @click="doSubmit_State('Runtime Error')"
              >
            ...
            <template #content>
              <div class="status-basic">
                ...
                <button
                  ...
                  @click="
                    () => {
                      record.judgeInfo.message = 'Waiting';
                      addSelectedKeys(record.id);
                    }
                  "
                >
              ...
              <div class="content-basic" style="display: flex">
                ...
                >
                  <p
                    ...
                    v-for="(item, index) in JSON.parse(record.outPut)"
                    :key="index"
                  >
                    实际输出{{ index }}：{{ item }}
                  </p>
        ...
        <template #questionId="{ record }">
          ...
          >
            <template #content
              ...
              ></template
            >
            <span>{{ record.questionVO?.title || "未知题目" }}</span>
          </a-tooltip>
        </template>
        <template #status="{ record }">
          <a-trigger
            :popup-translate="[100, -80]"
            auto-fit-position
            :unmount-on-close="false"
          >
            <a-space wrap style="margin-top: 10px; cursor: pointer">
              <span v-if="record.status === 2" style="color: #00af9b">
                成功
              </span>
              <span v-else-if="record.status === 1" style="color: #ffb800">
                判题中
              </span>
              <span v-else-if="record.status === 3" style="color: #ff2d55">
                失败
              </span>
            </a-space>
            <template #content>
          ...
          >
            <a-space wrap style="margin-top: 10px; cursor: pointer">
              <span
                ...
                style="font-weight: bold; color: #14c9c9"
                >Java
              </span>
              <span
                v-else-if="record.language === 'cpp'"
                style="font-weight: bold; color: #9fdb1d"
              >
                C ++
              </span>
              <span
...
const handleDelete = async () => {
  const promises = selectedKeys.value.map(async (id) => {
    const res =
      await QuestionSubmitControllerService.deleteQuestionSubmitUsingPost({
        id,
      });
    if (res.code === 0) {
...
const handleUpdate = async () => {
  const promises = selectedKeys.value.map(async (id) => {
    ...
    if (index !== -1) {
      const res =
        await QuestionSubmitControllerService.updateQuestionSubmitUsingPost(
          dataList.value[index]
        );
      if (res.code === 0) {
        return { id, success: true };
